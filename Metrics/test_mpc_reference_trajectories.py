...
# Convert path to a numpy array and transpose it
path_array = np.array(path).T

plt.subplot(2, 1, 1)
plt.plot(all_states[:, 0], all_states[:, 1], label='Actual Trajectory')
plt.plot(path_array[0], path_array[1], 'r--', label='Reference Trajectory')
plt.title('Drone Trajectory Versus Reference Trajectory')
plt.xlabel('X Position (m)')
plt.ylabel('Y Position (m)')
plt.legend()

# Save all_states data
np.save('all_states_sin.npy', all_states)

# Save path_array data
np.save('path_array_sin.npy', path_array)

# Create an array of times for the reference path
time_ref = np.linspace(0, 1, len(path_array[0]))

# Create an array of times for the actual path
...
# Interpolate the reference path to match the actual path
path_interp = np.vstack([interp_x(time_actual), interp_y(time_actual), interp_z(time_actual)])

# Now you can calculate the error as before
error = path_interp[:2] - all_states[:, :2].T
error_distance = np.linalg.norm(error, axis=0)

plt.text(0.2, 0.1, f"Mean Error Distance: {np.round(np.mean(error_distance),2)}", horizontalalignment='center', verticalalignment='center', transform=plt.gca().transAxes)
# print("Mean Error Distance:", np.mean(error_distance))
