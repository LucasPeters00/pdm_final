...
def add_obstacles_rrt_star_small_environment(): #function to add the obstacles for the simulation
    num_columns = 12  
    column_positions = [
        [0, 1, 0.8], [1, 1, 0.8], [2, 1, 0.8],
        [0, 2, 0.8], [1, 2, 0.8], [2, 2, 0.8],
    ...
    # Not needed for the RRT* algorithm
    # Add the sliding columns
    # sliding_column_ids = []
    # sliding_column_ids.append(p.loadURDF("urdf_files/sliding_column.urdf", [-1,1.5,.8], p.getQuaternionFromEuler([0, 0, 0])))
    # sliding_column_ids.append(p.loadURDF("urdf_files/sliding_column.urdf", [3,2.5,.8], p.getQuaternionFromEuler([0, 0, 0])))
    ...
    for i in range(num_columns): # Add the columns
        # p.loadURDF("urdf_files/column.urdf", column_positions[i], p.getQuaternionFromEuler([0, 0, 0])) # Not needed for the RRT* algorithm
        x = column_positions[i][0]
        y = column_positions[i][1]
        z = column_positions[i][2]
        radius = 0.3 #slightly larger then real, to have a safety margin.
...
def add_obstacles_rrt_star_large_environment(): #function to add the obstacles for the RRT* algorithm testing 
    num_columns = 96  
    column_positions = [
